...
namespace Thesis_backend.Controllers
{
    ...
    public class GameController : ThesisControllerBase
    {
        ...
        public async Task<IActionResult> GetScores([FromQuery] DateTime since)
        {
            ...
            List<GameScore> gameScores = await Database.GameScores.All.Include(u => u.Owner).Where(x => !x.Owner.Username.Contains("testt7GuSu") && x.AchievedTime > since).OrderByDescending(x => x.Score).Take(Config.LEADERBOARD_SIZE).ToListAsync();
