...
namespace Thesis_backend.Controllers
{
    ...
    public class TasksController : ThesisControllerBase
    {
        ...
        public async Task<IActionResult> GetTaskHistories()
        {
            ...
            List<TaskHistory> taskHistories = await Database.TaskHistories.All.Include(t => t.CompletedTask).Where(x => x.OwnerId == loggedInUserId).OrderByDescending(x => x.Completed.Ticks).Take(Config.TASK_HISTORY_SIZE).ToListAsync();
